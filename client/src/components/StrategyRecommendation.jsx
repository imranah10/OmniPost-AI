...
export default function StrategyRecommendation({ 
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setAutoIn((v) => (v == null ? null : v - 1)), 1000);
    return () => clearTimeout(t);
  }, [autoIn, isGeneratingCampaign]);
            ...
            {autoIn != null && (
              <div className="flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-2xl bg-gradient-to-r from-indigo-950/70 via-purple-950/50 to-pink-950/40 border border-indigo-500/50">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 border border-slate-700 text-[11px] font-semibold text-slate-200 transition cursor-pointer shrink-0"
                >
                  Customize instead
                </button>
              </div>
